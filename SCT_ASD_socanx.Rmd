...
```{r dawbabands}
bandtable <- table(sct.only$sophband,sct.only$asdband,sct.only$group8level)

sct.only$extraX<-1
w<-c(which(sct.only$trisomy==3),which(sct.only$trisomy==6))
sct.only$extraX[w] <-0


#NB unclear why some cases have NA for asdband. Must be missing data - checked: 
#Yes, these are cases where DAWBA only partially completed
#treat them as zero for ASD
w<-which(is.na(sct.only$asdband))
sct.only$asdband[w]<-0
sct.only$socanxvsasd<-sct.only$sophband-sct.only$asdband
#subtract one from the other to get a measure of bias to anxiety vs ASD

lorisk <-filter(sct.only,group8level<4) #analysis applies only to lorisk cases

#These are all pretty exploratory analyses
kruskal.test(lorisk$sophband~lorisk$group8level) 
kruskal.test(lorisk$asdband~lorisk$group8level) 
kruskal.test(lorisk$sophband~lorisk$extraX) #prediction is regarding extra X?
kruskal.test(lorisk$asdband~lorisk$extraX)
kruskal.test(lorisk$socanxvsasd~lorisk$extraX) #measure contrasting ASD/socanx 
beeswarm(lorisk$socanxvsasd~lorisk$group8level)
...
make.beeswarm <- function(mydata,groupcol,subgroupcol,colvalues,varlist,namelist,pngname,pngdim,
  ...
  for (i in 1:length(varlist)){
    ...
    if(addtext==1){
      text(2,mylims[1],'<------Low Bias------>',cex=1.5)
      text(5,mylims[1],'<------High Bias------>',cex=1.5)
    }
...
The DAWBA software codes the items relevant to different diagnoses into bands that indicate the likelihood of the child meeting criteria for a range of psychiatric diagnoses (Goodman et al, 2011), with the final diagnosis being made by a trained rater who assimilates all the quantitative and qualitative information. DAWBA has been used to obtain prevalence estimates for psychiatric disorders for a representative sample of over 10,000 children in the UK (Meltzer et al., 2000). Validity of DAWBA for diagnosis of autism has been shown to be acceptable (McEwen et al, 2016).
