...
my.dawba<-select(my.dawba1,record_id,female,trisomy,orig_famcode,bias,why_tested,asd,
                 ...
                 group8,group8level,ageband,sas_total,langfactor,alldsm,alldsm_factor)
...
load("xyz.Rdata") #loads in previously created randomised data

#Analysis 1: compare the 3 low bias SCT groups
mydata<-filter(pca.dat,group8level<4)
...
prevfile<-myp1 #if n.iter is > 0, this is ignored and new data will be generated
...
mydata <- filter(pca.dat,group8level %in% c(1:3,8))
gpcolumn<-which(colnames(mydata)=='bias') #low bias vs twin, so proxy for sct/twin
gpvalues<-unique(mydata$bias)
mytitle<-'Low Bias SCT vs Language Concern twins'
pngname <-'random99_chiplot_lowbias_twin.png'
prevfile<-myp3
myp3 <- do.permute(mydata,mycolrange,gpcolumn,gpvalues,n.iter,mylabs,
                   mytitle,pngname,prevfile,upperquant)

mydata <- filter(mydata,bicode2 !='ASD')
mydata <- filter(mydata,bicode2 !='ASD+SocAnx')

gpcolumn<-which(colnames(mydata)=='bias') #low bias vs twin, so proxy for sct/twin
gpvalues<-unique(mydata$bias)
mytitle<-'Low Bias SCT vs Language Concern twins, excluding autistic'
pngname <-'random99_chiplot_lowbias_twin_notASD.png'
prevfile<-myp4
myp4 <- do.permute(mydata,mycolrange,gpcolumn,gpvalues,n.iter,mylabs,
                   mytitle,pngname,prevfile,upperquant)

#since it took a while to make these randomised files (about 7 min each), we'll save them
#save(myp1,myp2,myp3,myp4, file = "xyz.Rdata")
#these variables can then be loaded in with 'load("xyz.Rdata")' and used for prevfile
...
3. We predicted that social impairment and autistic features in children with sex chromosome trisomies are consistent with the level of language impairment. To test this idea we compared the profile of social impairment and autistic features with that seen in a comparison group of children with language difficulties of unknown origin. 
...
In addition, one or both parents were asked to complete the online Development and Wellbeing Assessment (DAWBA){Goodman, 2000 #18146} in their own time. `r (n.K+n.L+n.M+n.N+n.O+n.P-1)` families of children with SCT (one with two affected children) complied with this request. The DAWBA was also completed by `r dawba_TD_Y+dawba_LI_Y` parents of twins. 
