...
sct.redcap <- 'SCTData_DATA_2018-11-30_1018.csv'
...
```{r beeswarm_srs, include=F,message=FALSE}
...
# varlist<-c('emotional','conduct','hyperactivity','peer','prosocial','sas_total')
# namelist<-c('Emotional','Conduct','Hyperactivity','Peer Relations',
#             'Prosocial','Social Aptitude')
# mylims<-c(0,30,0,0,0) #first 2 values are yaxis lims, then values for lines at mean -1sd and -2sd
# make.beeswarm(mydata,groupcol,subgroupcol,varlist,namelist,meanfile,pngname,pngdim,pngwidth,pngheight,
#                           groupcolor,mylims,dodiffmean,addtext)
# 
...
This 10-item scale forms part of the DAWBA and was designed to tap skills in social understanding and behaviour that vary substantially in the general population. The behaviours surveyed in SAS relate to impairments of social understanding characteristic of ASD rather than to social anxiety.
...
```
Meltzer et al (2000) reported prevalence rates for 'Pervasive Developmental Disorder' (encompassing ASD and SCD) of 0.5% for boys and 0.1% for girls. Compared with these figures, there are elevated rates in all three trisomies. We consider here just the Low Bias groups: the log odds ratio (with 95% confidence interval) is `r logoddsr.xxx` (`r ci.low.xxx` - `r ci.hi.xxx`) for XXX girls; `r logoddsr.xxy` (`r ci.low.xxy` - `r ci.hi.xxy`) for XXY boys; and `r logoddsr.xyy` (`r ci.low.xyy` - `r ci.hi.xyy`) for XYY boys.

*Need to check: are these the same children as were identified in the previous study?!*
