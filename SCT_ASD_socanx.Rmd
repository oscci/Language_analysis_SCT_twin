...
nicebandtab<-data.frame(matrix(NA, ncol=8, nrow=5)) 
colnames(nicebandtab)<-c('Band','Goodman et al','XXX-low','XXY-low','XYY-low',
...
Meltzp<-sum(goodmanband[3:5])/MeltzN

for (sct in 1:3){
  sctN<-sum(bandtable[,sct])
  sctaff<-sum(bandtable[3:5,sct])
...
All interviews with SCT cases were coded against DSM-IV criteria by the first author (AW), and a further 30 interviews were coded by a second independent rater (JK) to check on diagnostic agreement. The rater doing the coding was blind to the trisomy status of the child, although occasionally this was mentioned by parents in free text comments. Cases of disagreement were resolved by the last author (DVMB), who gave a final consensus rating that was used here. 

To address our questions of interest, we focused on DSM-IV diagnoses of Autism Spectrum Disorder (ASD) and Social Anxiety Disorder. ASD was coded as absent (0) or present (2), with a code of 1 given for significant autism symptoms that fell short of diagnostic criteria. Most of the latter group, which we refer to as the ASD- group, showed evidence of social and communicative difficulties, but did not have repetitive behaviours and restricted interests to a sufficient level to merit an ASD diagnosis. 
...
Our second prediction was that there would be a raised prevalence of SocAnx in those with an extra X chromosome. As shown in Table `r (thistab-1)`,there were `r numnames[socanx.xxy1+1]` with this diagnosis in the XXY-Low Bias group and `r numnames[socanx.xxy2+1]` in the XXY-High Bias group, as well as one child in the XYY-High Bias group. All but `r numnames[mytab[2,2]+1]` of these children had additional diagnoses of ASD- or ASD. In the epidemiological sample of Meltzer et al. (2000), the prevalence of SocAnx was 0.3% in girls and 0.4% in boys. Although the numbers of boys with XXY studied here is very small, the finding that 21% in the Low Bias group and 28% in the High Bias group met diagnostic criteria for this condition is well above the prevalence in the general population (log odds ratio for Low Bias group = `r round(logoddsr,2)`, 95% CI = `r ci.low`-`r ci.hi`). It may seem plausible that SocAnx would be a condition that develops with age, as the demands on children to function independently increase. However, there was no obvious association with age in this sample: the three XXY boys with SocAnx in the Low Risk group were aged 6, 10 and 14 years. DAWBA categorical diagnoses only detect more severe levels of impairment associated with burden to families. We therefore also examined symptoms of SocAnx reflected in the DAWBA band (Goodman et al, 2011), an ordinal scale computed from the profile of social anxiety symptoms. . 


Further information on subclinical features associated with an additional X or Y chromosome can be obtained from the profile of scores on the two questionnaires incorporated into the DAWBA: SDQ and the SAS. `r thisfig<-thisfig+1` Note that scores were converted to T-scores, where a high score indicates impairment. Figure `r thisfig` shows individual data points as well as means on the five SDQ scales and the SAS, with children meeting diagnostic criteria for ASD or SocAnx colour-coded.

![**Figure `r thisfig`**. *Beeswarm plots: SDQ and SAS scales for the three trisomy groups, subdivided by bias group. Symbol depicts age band (circle 5-10 yr, triangle 11-16 yr. Colour depicts diagnosis: grey = none; light blue = SocAnx; dark blue = Soc Anx + ASD-; pink = ASD-; red = ASD; purple = ASD+SocAnx )](beeswarm_sdq_DEC2018.png)
...
![**Figure `r thisfig`**. *Beeswarm plots: SDQ and SAS scales for the three trisomy groups, subdivided by bias group. Symbol depicts age band (circle 5-10 yr, triangle 11-16 yr. Colour depicts diagnosis: grey = none; light blue = SocAnx; dark blue = Soc Anx + ASD-; pink = ASD-; red = ASD; purple = ASD+SocAnx )](beeswarm_srs_2018.png)
