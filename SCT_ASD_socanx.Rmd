...
```{r data_reduction}
#retained for historical interest, but this was not v illuminating!
...
k.cols <- c("socaw_ss","soccog_ss","soccomm_ss","socmot_ss","autfeat_ss",
 ...
 for (k in mycolrange){
  # print(colnames(gp.sctlow[k]))
   myk1<- kruskal.test(gp.sctlow[,k]~gp.sctlow$group8level) # low bias only: compare trisomies
   myk2<- kruskal.test(gp.allsct[,k]~gp.allsct$bias) # compare all with vs without bias 
 
 allk1<-c(allk1,myk1$p.value)
 allk2<-c(allk2,myk2$p.value)
 }
```
 
```{r permutekruskalfunction}
 do.permute <- function(mydata,mycolrange,gpcolumn,gpvalues,n.iter){
 ...
 myp<-data.frame(matrix(NA,nrow=n.iter,ncol=length(mycolrange)))
 for (i in 1:n.iter){
...
```{r run.kruskal.permute}
 ...
 k.cols <- c("socaw_ss","soccog_ss","soccomm_ss","socmot_ss","autfeat_ss",
              "emotional","conduct","hyperactivity","peer","prosocial" ,"sas_total")
 mycolrange<-which(colnames(gp.sctlow) %in% k.cols)
 gpcolumn<-which(colnames(gp.sctlow)=='group8level')
 gpvalues<-unique(gp.sctlow$group8level)
  n.iter<-10000
 myp <- do.permute(mydata,mycolrange,gpcolumn,gpvalues,n.iter)
 w<-which(myp<min(allk1))
 perm.p<- length(w)/n.iter

 mydata<-gp.allsct
 gpcolumn<-which(colnames(gp.allsct)=='bias')
 gpvalues<-c(0,1)
  myp2 <- do.permute(mydata,mycolrange,gpcolumn,gpvalues,n.iter)
 w<-which(myp2<min(allk2))
 perm.p2<- length(w)/n.iter
