...
```{r permutekruskalfunction}
 do.permute <- function(mydata,mycolrange,gpcolumn,gpvalues,n.iter,mylabs,mytitle,pngname){
...
horizontal=TRUE,las=1,outcol='#FFFFFF',outcex=.1,
main='')
...
```{r get.kruskal.realdata}


k.cols <- c("socaw_ss","soccog_ss","soccomm_ss","socmot_ss","autfeat_ss",
              "emotional","conduct","hyperactivity","peer","prosocial" ,"sas_total")
 mycolrange<-which(colnames(pca.dat) %in% k.cols)
 gp.sctlow<-filter(pca.dat,group8level<4)
 gp.allsct <-filter(pca.dat,group8level<7)
 gp.scttwin <-filter(pca.dat,group8level%in%c(1:3,8)) #lowbias sct and langconc twins
 gp.scttwin2 <-filter(gp.scttwin,bicode2!='ASD')
 gp.scttwin2 <-filter(gp.scttwin2,bicode2!='ASD+SocAnx')
...
```{r run.kruskal.permute}
 ...
 pngname <-'random_chiplot_low_vs_hibias.png'
 myp <- do.permute(mydata,mycolrange,gpcolumn,gpvalues,n.iter,mylabs,
                   mytitle,pngname)
...
We had planned to use Multivariate Analysis of Variance (MANOVA) to compare groups on the SDQ and SRS scales, but many of the variables suffered from floor and ceiling effects or skew, and so nonparametric Kruskal-Wallis tests were used instead. Since the dependent variables were correlated, permutation was used to estimate probability of obtained results under the null hypothesis. A series of analyses was conducted to address the specific questions raised in the Introduction.

**Test of hypotheses 1-2**. The first two hypotheses were that there would be karyotype-specific differences in profiles, with autistic features more prominent in boys with XYY karyotype, and social anxiety in the XXX girls and XXY boys. To reduce the effects of ascertainment bias, these predictions were tested using data from the Low Bias groups only. Distributions of scores for the three karyotypes were compared on the five SDQ scales, the five SRS scales, and the Social Aptitudes Scale. We predicted that the XYY boys would score lower than the XXX and XXY groups on the two scales measuring positive aspects of social behaviour (SDQ Prosocial and SAS), and higher (i.e., more impaired) on the SRS scales. We further predicted that the XXX girls and XXY boys would score higher (more impaired) on the SDQ emotional scale. `r thistab<-thistab+1`. Results are summarised in Table `r thistab`. The only scale to show an effect in line with prediction was the SDQ prosocial scale (p = `r round(min(allk),3)`), but permutation testing (10000 iterations) indicated that a result this extreme would occur by chance with p  = `r round(perm.p,3)`. Given the small sample sizes, the power is low to detect subtle effects, but it is worth noting that the lack of difference reflects wide variation within each of the karyotype groups, with some children scoring well within normal limits. 

**Test of hypothesis 3**. Hypothesis 3 predicts that children in the High Bias group will have more evidence of impairment than those in the Low Bias group. Again, multiple Kruskal Wallis tests were used to test this prediction (with the three karyotypes combined)
