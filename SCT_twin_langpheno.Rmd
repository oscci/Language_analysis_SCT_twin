...
library(officer)
library(stargazer)
testbattery<-read.csv('testbattery.csv') #used for Table in word doc
...
manova.df<-data.frame(matrix(NA,nrow=4,ncol=7))
...
mycomparison<-paste0('Low bias: XXX (',mytab[1],
                      ') vs XXY (',mytab[2],
                       ') vs XYY (',mytab[3],')')
...
mycomparison<-paste0('Trisomies (',mytab[1],') vs. Lang concern twins (',mytab[2],'): aged 6-11')
...
gp6.fit <- manova(sct2.matrix ~ trisomy.only$bias*trisomy.only$group8xlevel+trisomy.only$mo_educ+trisomy.only$deprivz)
...
mycomparison<-'3 trisomy x 2 bias group'
...
mycomparison<-paste0('XXX (',mytab[1],'), XXY (',mytab[2],'),XYY (',mytab[3],')')
...
mycomparison<-'All SCT vs Lang concern twins'
effectnames<-c('Group','Mo. education','Depriv. index')
manova.ccc.df<-manova.function(mycomparison,effectnames,offset,manova.stats,manova.ccc.df)
...
Table 2 shows the characteristics of the trisomy and twin groups on background variables: age, maternal educational level and standardized deprivation index.

```{r table2, echo=FALSE}

ft2<-regulartable(mybkgtab2)
ft2 <- width(ft2, width = c(2,1,1,1,1))
ft2
```
1.	**Comparison of three trisomy groups** (low risk of bias only)  

**Psychometric tests**
As a first step, data from the psychometric test variables were plotted to see the range and distribution of scores for the three low bias trisomy groups, as shown in the red points in Figure 3. These plots exclude children with missing data on any test. The continuous horizontal bars show the median for each group.  All data have been scaled to mean of 100 and SD 15. The yellow shaded area shows the range covered by mean +/- 1 SD for the No Concerns twins. The distribution of blue points, corresponding to the Language Concerns twins will be discussed below.  
...
![Figure 3: Distributions of scores on psychometric tests for the Low Bias trisomy groups and the Language Concerns Comparison group](beeswarm_lang.png)

MANOVA was used to test whether there were reliable differences between the three trisomy groups on the 14 test measures, after taking covariates into account.  No effect of trisomy type was found (see Table 2).

```{r table3, echo=FALSE}

regulartable(manova.df)
```
