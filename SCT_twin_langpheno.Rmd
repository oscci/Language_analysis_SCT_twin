...
#Use latest version
sct.redcap <- 'SCTDATA_DATA_2018-08-08_1425.csv'
...
keepcols <- c('record_id','female','age_at_test','partial_testing','pass_hearing','wasi_matrices_ss','wasi_block_design_ss','wasi_vocab_ss','wdck_jhsn_ss',
              ...
              'ccc_g','ccc_h','ccc_i','ccc_j','ccc2_consistent','mo_educ','fa_educ','deprivation_index',
...
#Explore effect of removing top decile.
#Better might be to weight comparison group by dep index
pirateplot(deprivation_index~group8,data=all.short[all.short$deprivation_index<29475,],main='Deprivation index: high is good, excluding top decile')

#Depriv index to decile
all.short$depriv_decile <- cut(all.short$deprivation_index, 
                       breaks = c(0,3275,6550,9825,13100,16375,19650,22925,26200,29475,Inf),
                       labels = seq(1:10), 
                       right = TRUE)
all.short$depriv_decile<-as.integer(all.short$depriv_decile)
deciletab<-table(all.short$depriv_decile,all.short$group8)
pirateplot(depriv_decile~group8,data=all.short)
```
##CCC-2 analysis
