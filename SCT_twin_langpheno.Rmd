...
for (i in 1:14){
  mytitle<-longname[i]
  beeswarm(jitter(lobias.sct2[,nurange[i]],3)~mycontrast , data = lobias.sct2, xlab='Group',ylab='Scaled score',
           pch = 16,cex=1,col=c(2,4),main=mytitle,ylim=c(50,140),spacing=.8)
  bxplot(lobias.sct2[,nurange[i]]~mycontrast , data = lobias.sct2, probs = 0.5, col = 1,add=TRUE) #adds medians
}
dev.off()
...
cccbit<-all.short[,c(1,(w-2):(w+10),x,y,2,3)]#2 before ccc_a is GCC and SIDC
...
#remove those out of age range
ccc.gp2<-filter(ccc.gp2,Age>71)
ccc.gp2<-filter(ccc.gp2,Age<144)
...
#Comparisons of high bias vs remainder
