...
#Use latest version
sct.redcap <- 'SCTDATA_DATA_2018-08-09_1107.csv'
#updated to include code indicating whether participated in prior study
twin.redcap <- 'TwinsData_DATA_2018-08-09_1134.csv' #updated to include ccc2_consistent variable
...
```
##Plot language test and reading data
First do plots showing whole distribution

```{r langmeasures}


langcogcols<-c("wasi_matrices_ss" ,"wasi_block_design_ss", "wasi_vocab_ss" ,
               "wdck_jhsn_ss" , "sent_rep_ss" , "nonword_rep_ss", 
               "nara_acc_ss" ,"nara_comp_ss","nara_rate_ss" ,"towre_words_ss",
               "towre_nonwords_ss","phab_pic_ss","phab_digit_ss",
               "oromotor_ss","group8","group8level","nepsy_oromotor_seq_raw","age_at_test")

langcog<-dplyr::select(all.short,langcogcols)
#Make shorter names for plotting
colnames(langcog)<-c('Matrices','Blocks','Vocab','Comprehnsn','SentRep','NwdRep',
                     'ReadAcc','ReadComp','ReadRate','TOWREwd','TOWREnwd',
                     'PicName','DigitName','Oromotor','Group','Grouplevel','Oromotor_raw','Age')
...
```
#Oromotor adjustment
Preliminary check shows that oromotor norms seem v odd. Even TD twins have low means.
The norming was based on the percentile scores in the test manual.
...
Seems preferable to norm this test against the TD twins - esp. as they look v normal on everything else.
...
#For twins, it looks as if there's very little relationship with age.
#Could just use mean and SD to make a z-score from raw score - at least for initial analysis
temp<-filter(langcog3,Group=='TD') #select TD only
cor(temp$Age,temp$Oromotor_raw)
hist(temp$Oromotor_raw)
mymean<-mean(temp$Oromotor_raw)
mysd<-sd(temp$Oromotor_raw)

all.short$oromotor_ss2<-round(100+15*(all.short$nepsy_oromotor_seq_raw-mymean)/mysd,0)
all.short$oromotor_ss2[all.short$oromotor_ss2<55]<-55
#We'll use this for all subsequent analyses
#Need to update value in langcog2
langcog2$Oromotor<-round(100+15*(langcog2$Oromotor_raw-mymean)/mysd,0)
langcog2$Oromotor[langcog2$Oromotor<55]<-55
```

#Now do a grid of beeswarm plots

```{r beeswarmgrid}
longname<-c('Matrices','Block Design','Vocabulary','Comprehension','Sentence Repetition','Nonword Repetition',
                     'Reading Accuracy','Reading Comprehension','Reading Rate','Speeded word reading','Speeded nonword reading',
                     'Rapid Naming: Pictures','Rapid Naming: Digits','Oromotor')
