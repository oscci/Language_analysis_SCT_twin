...
grouplevel<-sort(unique(mydata[,groupcol])) #list of group values in numeric order
...
for (i in 1:length(varlist)){
  ...
  for (g in 1:ngroup){
    ...
    #plot full lines for means for age-matched groups, i.e ageband2
    if(dodiffmean==1){
...
wt<-which(lobias.sct2$group8level==8)
...
namelist<-c('PIQ','Language','Verbal memory','Reading')
meanfile<-twin.TD
...
dodiffmean<-1
mydata<-lobias.sct2
make.beeswarm(mydata,groupcol,subgroupcol,varlist,namelist,meanfile,pngname,pngdim,
                          groupcolor,mylims,dodiffmean)
```
...
for (i in 1:length(longname)){
  ...
  beeswarm(jitter(trisomy.only[,nurange[i]],3)~group8level , data = trisomy.only, xlab='Group',ylab='Scaled score',spacing=.8,labels='',
           ...
           ylim=c(55,140))
  bxplot(trisomy.only[,nurange[i]]~group8level, data = trisomy.only, probs = 0.5, 
...
summary(lowdld6.aov)
cutofflang.tab<-table(trisomy.only$Ntestlow,trisomy.only$group8level)
cutofflang.tab
...
ccc.gp.sctlow<-filter(ccc.data,group8level<4)

#check multivariate normality for each group
for (i in 1:3){
  ccc.matrix <- as.matrix(ccc.gp.sctlow[ccc.gp.sctlow$group8level==i,cccnurange])
  print(paste('Check multivariate normality: group',i))
...
biasgp.ccc<-filter(ccc.data,group8level<7)
#check multivariate normality for each group
for (i in 1:2){
