...
all.short$deprivation_index[w]<-32850/2
#Assign avg score to those outside england where no dep index
all.short$deprivz<-qnorm(all.short$deprivation_index/32850)
...
table(all.short$group8,all.short$gcclow)
```
...
for (i in 1:length(w)) {
 ...
 all.short$langlow_count[-x]<-all.short$langlow_count[-x]+1
}
...
langlowcounttab<-table(all.short$group8,all.short$langlow_count)
...
![Figure 5: Mean scores on the three CCC-2 clusters for trisomy and comparison groups. For trisomies, continuous lines show mean for whole group, and dotted lines show means for children aged 6 to 11 years.](beeswarm_ccc.gp.png)
