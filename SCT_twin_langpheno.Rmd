...
#######################################################
#? already achieved with pheno_include?
...
missing.tab<-table(all.short$langdata_complete,all.short$group8xlevel)
...
colnames(mybkg.tab2)<-c('Group','N','Age (mo)','Mo. educ.','Depriv. index')
...
#alldatabit<-pointcol[(1+lobias.sct$langdata_complete)] #orig values are 0 and 1, so add 1
...
do.manova <- function(mydata,mycolrange,gpcolumn,gpvalues,covcolumns){
  ...
  mymanova.out<-summary(mymanova.fit, test="Wilks")
...
gpvalues<-c(1,2,3)
covcolumns<-which(colnames(gp.sctlow) %in% c('mo_educ','deprivz'))
mymanova.sum<-do.manova(gp.sctlow,psychrange,gpcolumn,gpvalues,covcolumns)
...
gpvalues<-c(0,1)
covcolumns<-which(colnames(gp.sctlow) %in% c('mo_educ','deprivz'))
mymanova.sum<-do.manova(lobias.agematch,psychrange,gpcolumn,gpvalues,covcolumns)
...
psychname<-c('PIQ','Language','Verbal memory','Reading')
par(mfrow=c(2,2)) #2 row and 2 columns
par(mar=c(5.1,4.1,4.1,0.2))
for (i in 1:length(psychrange)){
  mytitle<-psychname[i]
  beeswarm(jitter(lobias.sct2[,psychrange[i]],3)~is_twin , data = lobias.sct2, xlab='Group',ylab='Scaled score',
           pch = 16,cex=1,col=c(2,4),main=mytitle,ylim=c(50,140))
  bxplot(lobias.sct2[,psychrange[i]]~is_twin , data = lobias.sct2, probs = 0.5, col = 1,add=TRUE) #adds medians
}
dev.off()
...
psych.vars<-rowMeans(lobias.sct2[psychrange])
cor(lobias.sct2$mo_educ,psych.vars)
moedreg<-lm(psych.vars~lobias.sct2$mo_educ)
plot(lobias.sct2$mo_educ,psych.vars)
...
moedregt<-lm(psych.vars[wt]~lobias.sct2$mo_educ[wt])
...
for (i in 1:2){
  sctbias.matrix <- as.matrix(trisomy.only[trisomy.only$bias==(i-1),psychrange])
  #transpose cols and rows for mshapiro.test !
  print(paste('Bias group =',(i-1)))
  print(mshapiro.test(t(sctbias.matrix)))
}


sctbias.matrix <- as.matrix(trisomy.only[,psychrange])
gpbias.fit <- manova(sctbias.matrix ~ trisomy.only$bias+trisomy.only$mo_educ+trisomy.only$deprivz)
mymanova.bias<-summary(gpbias.fit, test="Wilks")
print('Manova on the psychometric measures for the Low Bias SCT groups vs High Bias')
...
mycomparison<-'C.2 bias groups'
effectnames<-c('Bias','Mo. education','Depriv. index')
offset<-nrow(manova.dataframe)
manova.stats<-mymanova.bias$stats
manova.dataframe<-manova.write(mycomparison,effectnames,offset,manova.stats,manova.dataframe)
manova.dataframe[(2+offset),1]<-paste0('- Ns Low bias: XXX (',mytab[1],'), XXY (',mytab[2],'), XYY (',mytab[3],')')
manova.dataframe[(3+offset),1]<-paste0('- Ns High bias: XXX (',mytab[4],'), XXY (',mytab[5],'), XYY (',mytab[6],')')  
summary.aov(mymanova.bias.fit) #- if you want to inspect individual language measures
...
colA<-which(colnames(ccc.gp.sctlow)=='ccc_a')
cccrange<-colA:(colA+9)
#Not multivariate normal. Take avgs of A-D,E-H and I-J to make more normal
ccc.data$ccc_struct<-rowMeans(ccc.data[,cccrange[1:4]],na.rm=TRUE)
ccc.data$ccc_prag<-rowMeans(ccc.data[,cccrange[5:8]],na.rm=TRUE)
ccc.data$ccc_autfeat<-rowMeans(ccc.data[,cccrange[9:10]],na.rm=TRUE)
cccnurange<-(ncol(ccc.data)-2):ncol(ccc.data)

ccc.gp.sctlow<-filter(ccc.data,group8xlevel<4)

#check multivariate normality for each group
for (i in 1:3){
  ccc.matrix <- as.matrix(ccc.gp.sctlow[ccc.gp.sctlow$group8xlevel==i,cccnurange])
  print(paste('Check multivariate normality: group',i))
  print(mshapiro.test(t(ccc.matrix)))
}


ccc.mat <- as.matrix(ccc.gp.sctlow[,cccnurange])
fit.cccsct <- manova(ccc.mat ~ ccc.gp.sctlow$group8+ccc.gp.sctlow$mo_educ+ccc.gp.sctlow$deprivz)
summary(fit.cccsct, test="Wilks")

mytab<-table(ccc.gp.sctlow$group8)
...
nsct<-nrow(ccc.gp.sctlow)
ccc.sct.dld<-rbind(ccc.gp.sctlow,temp)
ccc.sct.dld$group2<-1
ccc.sct.dld$group2[1:nsct]<-2
#check multivariate normality for each group
for (i in 1:2){
  ccc.matrix <- as.matrix(ccc.sct.dld[ccc.sct.dld$group2==i,cccnurange])
  print(paste('Check multivariate normality: group',i))
  print(mshapiro.test(t(ccc.matrix)))
}

Y <- as.matrix(ccc.sct.dld[,cccnurange])
fitccc2 <- manova(Y ~ ccc.sct.dld$group2+ccc.sct.dld$mo_educ+ccc.sct.dld$deprivz)
...
biasgp.ccc<-filter(ccc.data,group8xlevel<7)
#check multivariate normality for each group
for (i in 1:2){
  ccc.matrix <- as.matrix(biasgp.ccc[biasgp.ccc$bias==(i-1),cccnurange])
  print(paste('Check multivariate normality: bias group',(i-1)))
  print(mshapiro.test(t(ccc.matrix)))
}

Ybias <- as.matrix(biasgp.ccc[,cccnurange])
fitcccbias <- manova(Ybias ~ biasgp.ccc$bias+temp1$mo_educ+temp1$deprivz)
summary(fitcccbias, test="Wilks")
summary.aov(fitcccbias) #- if you want to inspect individual language measures
manova.stats<-summary(fitcccbias, test="Wilks")$stats
offset<-nrow(manova.ccc.df)
mycomparison<-'C. Bias '
effectnames<-c('Bias','Mo. education','Depriv. index','Bias x Trisomy')
manova.ccc.df<-manova.write(mycomparison,effectnames,offset,manova.stats,manova.ccc.df)



#GCC and SIDC - just do anova
aov.gcc<-aov(biasgp.ccc$gcc~biasgp.ccc$bias)
summary(aov.gcc)

aov.scdi<-aov(biasgp.ccc$scdi~biasgp.ccc$bias)
summary(aov.scdi)
```
...
beeswarm(biasgp.ccc$gcc~biasgp.ccc$trisomy*biasgp.ccc$bias, xlab='Group',ylab='GCC' ,
         pch = 16,cex=.9,col=c(2,2,2,1,1,1),ylim=c(0,120))
bxplot(biasgp.ccc$gcc~biasgp.ccc$bias, probs = 0.5, col = 1,lty=1,add=TRUE) #adds medians for all cases with dotted line
...
#--------------------------------------------------------------------------------#
mytab1<-table(all.short$langdata_complete,all.short$group8xlevel)
