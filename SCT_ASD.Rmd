...
mydata<-read.csv(paste0(dir,"SCTData_DATA_2017-10-30_0855.csv"))
...
## Participants
Participants included 43 girls with trisomy X, 45 boys with Klinefelter syndrome, and 52 boys with XYY. As shown in Figure 1, 68 cases were recruited from National Health Service Clinical Genetics centres, and the remainder from two support groups: Unique: the Rare Chromosome Support Group, and the Klinefelter Syndrome Association, or from self-referral via social media. A criterion for inclusion was that the child was aware of their trisomy status. As in our previous study (Bishop et al., 2011), in the analysis we distinguish between cases where the trisomy was discovered on prenatal screening, and those identified later in childhood. We also asked parents specifically about the reason for genetic testing, and distinguished a subset of children where this was part of an assessment of either behavioural or neurodevelopmental problems. The postnatally identified group included a preponderance of children whose chromosome aneuoploidy only came to light because of behavioural or developmental problems (XXX: N = 12/18, XXY: N = 26/34 and XYY: N = 32/40), and so cannot be used to give an estimate of prevalence of disorders. 
![<b>Figure 1</b>. Flowchart showing numbers of children with each karyotype in the study, in relation to whether prenatally or postnatally diagnosed, and whether DAWBA was completed.](/Users/dorothybishop//Dropbox/ERCadvanced/project SCT analysis/SCT_ASD_analysis/Project_Files/plots/flowout.png)
...
In addition, one or both parents were asked to complete the online Development and Wellbeing Assessment (DAWBA) in their own time. 86 families complied with this request. The DAWBA gives information on likelihood of the child meeting criteria for a range of psychiatric diagnoses, but the final diagnosis is made by a trained rater who assimilates all the information and evaluates it against both ICD and DSM criteria. For this project, all interviews were coded by the first author, and a further xx interviews were coded by a second independent rater to check on diagnostic agreement. The raters doing the coding was blind to the trisomy status of the child, *although in some cases the parent mentioned this?*.

### Language, literacy and cognitive assessments
\

All children were seen for a neurocognitive assessment, using the test battery shown in Table 1. Hearing was screened using a xxx audiometer, with the screen being passed if reliable responses were elicited at 30 dB or less to pure tone frequencies of 500 Hz, 1000 Hz, 2000 Hz and 4000 Hz in the better ear. In addition, handedness and language laterality were assessed, the latter involving a child-friendly functional transcranial Doppler ultrasound paradigm (Groen et al, 2012). Results from laterality assessments will be described in another publication.
...
file.loc<-"~/Dropbox/ERCadvanced/Project SCT analysis/SCT_ASD_analysis/Project_Files/writeup/"
filename<-c("SCT_Table1_for_kable.csv") 
tab1data<-data.frame(read_csv(paste0(file.loc,filename)))
...
Table 2 shows the full set of independent and dependent variables considered in the analysis. All analyses were run separately for the prenatally identified and postnatally identified cases.
```{r, echo=FALSE} 
### Table 2
# options(knitr.kable.NA = '')
# file.loc<-"~/Dropbox/ERCadvanced/Project SCT analysis/SCT_ASD_analysis/Project_Files/writeup/"
# filename<-c("SCT_Table2_for_kable.csv") 
# tab2data<-data.frame(read_csv(paste0(file.loc,filename)))
# kable(tab2data,caption='Independent and Dependent Variables')
```


# Results
Figure 2 shows the T-scores on the total SRS for individual children grouped by trisomy type and whether diagnosis was prenatal or postnatal. A high score on the SRS corresponds to impairment. The recommended cutoff for suspected ASD is 60, though in some studies covered in the introduction, a more stringent cutoff of 75 was used.It is apparent just from inspection that, relative to published norms, there is a high rate of impairment on the SRS in children with sex chromosome trisomies, but that this is considerably more striking for those diagnosed postnatally. A regression analysis predicting SRS T-score from Trisomy, Diagnosis (prenatal vs postnatal) and age confirmed that the only factor accounting for variance in the T-score was time of diagnosis.
...
lmmdata<-select(mydata,Age,Trisomy,Diagnosis,SRS)
...
os1=get_os()
ifelse(os1=="windows",source(paste0("c:/Users/",Sys.info()["login"],"/Dropbox/Project SCT analysis/SCT_ASD_analysis/Project_files/Analysis/analysis_script.R")),source("~/Dropbox/ERCadvanced/Project SCT analysis/SCT_ASD_analysis/Project_files/Analysis/SCT_SRS_pirateplot.R"))
